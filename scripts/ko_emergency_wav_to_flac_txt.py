...
DEFAULT_ROOT = Path("/usr/local/corpus/ko/emergency_situation_voice_sound")
...
def add_candidate(candidates: List[Path], seen: set, directory: Path, stem: object) -> None:
    if not stem:
        return
    ...
    if isinstance(annotations, list):
        for annotation in annotations:
            if isinstance(annotation, dict):
                add_candidate(candidates, seen, directory, annotation.get("audio_id"))

    audio = record.get("audio") or {}
    if isinstance(audio, dict):
        file_name = audio.get("fileName")
        add_candidate(candidates, seen, directory, f"{file_name}_label" if file_name else None)
        add_candidate(candidates, seen, directory, file_name)

    add_candidate(candidates, seen, directory, json_path.stem)
    add_candidate(candidates, seen, directory, f"{json_path.stem}_label")
    return candidates
    ...
    if isinstance(annotations, list):
        for annotation in annotations:
            if not isinstance(annotation, dict):
                continue
            note = clean_text(str(annotation.get("note", "")).strip())
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Convert emergency_situation_voice_sound wav files to mono flac "
            "in-place and write same-stem txt files from annotations[].note."
        )
    )
    parser.add_argument(
        "dataset_root",
        ...
        help=f"Dataset root or unzip dir. Default: {DEFAULT_ROOT}",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
